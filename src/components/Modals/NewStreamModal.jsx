...
export default function NewStreamModal({
  ...
  // --- PROPS BARU DARI DASHBOARDCLIENT ---
  streamCategory, // State baru untuk kategori
  setStreamCategory,
  isMadeForKids, // State baru untuk audiens
  setIsMadeForKids,
  enableLiveChat, // State baru untuk live chat
  setEnableLiveChat,
  enableLiveChatRecording, // State baru untuk rekaman live chat
  setEnableLiveChatRecording,
  enableLiveChatHighlights, // State baru untuk ringkasan live chat
  setEnableLiveChatHighlights,
  liveChatParticipantMode, // State baru untuk mode peserta
  setLiveChatParticipantMode,
  enableReactions, // State baru untuk reaksi
  setEnableReactions,
  // --- AKHIR PROPS BARU ---
}) {
  const [liveDurationOption, setLiveDurationOption] = useState('1_day');
  const [customLiveHours, setCustomLiveHours] = useState(1);
  ...
  const handleCalculatePrice = async () => {
    ...
    const fileSizeGB = firstFile.size / (1024 * 1024 * 1024);

    const totalLiveHours = getLiveHours();
    const originalResolutionPlaceholder = "360p";
    ...
    try {
      const response = await fetch('/api/calculate-price', {
        ...
        body: JSON.stringify({
          ...
          originalResolution: originalResolutionPlaceholder,
        }),
      });
      const data = await response.json();
      if (response.ok) {
        setCalculatedPriceDetails(data.details);
      } else {
  ...
  const isCalculatePriceButtonDisabled = useMemo(() => {
    return (
      ...
      !streamTitle.trim() ||
      (liveDurationOption === 'custom' && customLiveHours <= 0) ||
      isCalculatingPrice ||
      isUploadingFile
    );
  ...
  const isStartStreamButtonDisabled = useMemo(() => {
    return !calculatedPriceDetails || isCalculatingPrice || isUploadingFile || !streamTitle.trim(); // Judul tetap wajib
  }, [calculatedPriceDetails, isCalculatingPrice, isUploadingFile, streamTitle]);
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">Audiens (Wajib)</label>
                <div className="flex items-center mb-2">
                  <input
                    ...
                    className="mr-2"
                  />
                  <label htmlFor="madeForKidsYes" className="text-sm text-gray-700">Ya, ini dibuat untuk anak-anak</label>
                </div>
                <div className="flex items-center">
                  <input
                    ...
                    className="mr-2"
                  />
                  <label htmlFor="madeForKidsNo" className="text-sm text-gray-700">Tidak, ini tidak dibuat untuk anak-anak</label>
                </div>
              </div>

              <div className="col-span-2 p-4 border rounded-md bg-white">
                <h4 className="text-md font-semibold text-gray-800 mb-3">Pengaturan Live Chat</h4>
                <div className="flex items-center mb-2">
                  <input
                    type="checkbox"
                    id="enableLiveChat"
                    ...
                    className="mr-2"
                  />
                  <label htmlFor="enableLiveChat" className="text-sm text-gray-700">Aktifkan Live Chat</label>
                </div>
                {enableLiveChat && (
                  <>
                    <div className="flex items-center mb-2 ml-4">
                      <input
                        type="checkbox"
                        id="enableLiveChatRecording"
                        checked={enableLiveChatRecording}
                        onChange={(e) => setEnableLiveChatRecording(e.target.checked)}
                        className="mr-2"
                      />
                      <label htmlFor="enableLiveChatRecording" className="text-sm text-gray-700">Aktifkan Rekaman Live Chat</label>
                    </div>
                    <div className="flex items-center mb-2 ml-4">
                      <input
                        type="checkbox"
                        id="enableLiveChatHighlights"
                        checked={enableLiveChatHighlights}
                        onChange={(e) => setEnableLiveChatHighlights(e.target.checked)}
                        className="mr-2"
                      />
                      <label htmlFor="enableLiveChatHighlights" className="text-sm text-gray-700">Aktifkan Ringkasan Live Chat</label>
                    </div>
                    <div className="mb-2 ml-4">
                      <label className="block text-sm font-medium text-gray-700">Mode Peserta</label>
                      <select
                        value={liveChatParticipantMode}
                        onChange={(e) => setLiveChatParticipantMode(e.target.value)}
                        className="mt-1 block w-full p-2 border rounded-md"
                      >
                    ...
                    <div className="flex items-center ml-4">
                      <input
                        ...
                        className="mr-2"
                      />
                      <label htmlFor="enableReactions" className="text-sm text-gray-700">Aktifkan Reaksi Live</label>
                    </div>
                  </>
                )}
              </div>
